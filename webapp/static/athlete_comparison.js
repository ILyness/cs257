...
function updateDividerNumbers() {
  const numbers = container.querySelectorAll(".divider-number");
  ...
  <form id="athleteSearchForm_${index}" class="mb-4" method="GET" action="/api/search">
    ...
    <div class="row mb-4">
      <div class="col">
        ...
        <input type="text" class="form-control" name="mark" id="mark_${index}" placeholder="e.g. 10.75" />
      </div>
    </div>
  ...
  if (form) {
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        contentWrapper.removeChild(searchFormContainer);

        const athleteListDiv = document.createElement("div");
        athleteListDiv.className = "athlete-list p-3 text-white overflow-auto";

        if (result.error) {
          athleteListDiv.innerHTML = `<p class="text-danger">Error: ${result.error}</p>`;
        } else if (result.athletes && result.athletes.length > 0) {
          athleteListDiv.innerHTML = `<h5>Search Results (${result.athletes.length} athletes):</h5>`;

          result.athletes.forEach(a => {
          ...
          `;
            ...
            athleteDiv.addEventListener("click", () => {
              contentWrapper.removeChild(athleteListDiv);
              if (contentWrapper.contains(backBtnForList)) contentWrapper.removeChild(backBtnForList);
          ...
          athleteListDiv.innerHTML = "<p>No athletes found matching your criteria.</p>";
        }

        const backBtnForList = document.createElement("button");
        backBtnForList.type = "button";
        backBtnForList.id = "backBtn_" + index;
        ...
        backBtnForList.addEventListener("click", () => {
          if (contentWrapper.contains(athleteListDiv)) contentWrapper.removeChild(athleteListDiv);
          if (contentWrapper.contains(backBtnForList)) contentWrapper.removeChild(backBtnForList);
